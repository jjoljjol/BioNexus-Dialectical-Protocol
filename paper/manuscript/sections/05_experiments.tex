...
\subsection{Baseline Methods (Extended)}
To rigorously evaluate the Dialectical Protocol, we compared it against several architectural baselines:
\begin{enumerate}
    \item \textbf{Genetic Programming (DEAP)}: Standard tree-based GA with subtree crossover (Pop: 50, Gen: 50).
    \item \textbf{RL-based Optimization (PPO)}: Reward assigned for compilation success and accuracy gain on test suites.
    \item \textbf{Constitutional AI (Fixed)}: Recursive self-improvement governed by static principles (e.g., ``Optimize speed'').
    \item \textbf{LangChain Multi-Agent (Static)}: Fixed-role agents without the ability to mutate their own logic.
\end{enumerate}
